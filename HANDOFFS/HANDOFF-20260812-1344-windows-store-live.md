...
- **`1.0.2.0` was uploaded the same day and is STILL IN CERTIFICATION.** This is
  the only open item. See *Next steps*.
...
1. **Wait for the 1.0.2.0 certification result email** to
   `nikki.19972010@hotmail.com`. There is **no way to poll this** — no
   Outlook/Hotmail MCP is connected, and Partner Center needs her login.
   Nikki must say so.
   - **If it passes:** nothing to do. The listing updates itself and the
     reward-code copy becomes correct. Update `START-HERE.md` and the memory
     file `project_windows_store_1_0_2.md`.
   - **If it fails:** ask her to forward the report, then fix and resubmit
     using `store\SUBMIT.md` **A5**.
